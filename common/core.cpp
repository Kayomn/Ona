...
namespace Ona {
	...
	bool String::EndsWith(String const & string) const {
		if (string.Length() < this->Length()) {
			Slice<uint8_t const> const a = this->Bytes();
			Slice<uint8_t const> const b = string.Bytes();

			return a.Sliced((a.length - b.length), b.length).Equals(b.Bytes());
		}
